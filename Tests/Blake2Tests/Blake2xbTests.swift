...
final class Blake2xbTests: XCTestCase {
    ...
    func testSimpleApi() {
        for (_in, key, hash) in BLAKE2XB_KAT {
            let computed = try? Blake2.hash(.b2xb, size: hash.count, bytes: _in, key: key)
    ...
    func testStreamingApi() {
        ...
        for step in 1..<BLAKE2B_BLOCKBYTES {
            for outlen in 1...256 {
                let (_, key, expected) = BLAKE2XB_KAT[outlen - 1]
                let oBlake2 = try? Blake2(.b2xb, size: outlen, key: key)
